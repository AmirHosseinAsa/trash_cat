...
public class CharacterInputController : MonoBehaviour
{
    ...
    protected void Update()
    {
        ...
        if (Input.GetAxisRaw("Horizontal") == -1 && TutorialMoveCheck(0) && lastHor != -1)
        {
            lastHor = -1;
            StartCoroutine(SetDefaultHor());
            ChangeLane(-1);
        }
        else if (Input.GetAxisRaw("Horizontal") == 1 && TutorialMoveCheck(0) && lastHor != 1)
        {
            lastHor = 1;
            StartCoroutine(SetDefaultHor());
            ChangeLane(1);
        }
        else if (Input.GetAxisRaw("Vertical") == 1 && TutorialMoveCheck(1) && lastVer != 1)
        {
            lastVer = 1;
            StartCoroutine(SetDefaultVer());
            Jump();
        }
        else if (Input.GetAxisRaw("Vertical") == -1 && TutorialMoveCheck(2) && lastVer != -1)
        {
            lastVer = -1;
            StartCoroutine(SetDefaultVer());
            if (!m_Sliding)
    ...
    private IEnumerator SetDefaultHor()
    {
        yield return new WaitForSeconds(.4f);
        lastHor = 0;
    }

    private IEnumerator SetDefaultVer()
    {
        yield return new WaitForSeconds(.4f);
        lastVer = 0;
    }
